/// @function camera_3d_create(fov, aspect, znear, zfar, xfrom, yfrom, zfrom, xto, yto, zto, xvec, yvec, zvec, velocity, width, height)
/// @param fov
/// @param aspect
/// @param znear
/// @param zfar
...
{
	var _camera = instance_create_layer(global.screen_width/2, global.screen_height/2, Layer_System, OBJ_Camera3D);

	if(argument_count >= 4)
	{
		_camera.camera_fov = argument[0];
		_camera.camera_aspect = argument[1];
		_camera.camera_znear = argument[2];
		_camera.camera_zfar = argument[3];
	...
	{
		_camera.camera_xfrom = argument[4];
		_camera.camera_yfrom = argument[5];
		_camera.camera_zfrom = argument[6];

		_camera.camera_xto = argument[7];
		_camera.camera_yto = argument[8];
		_camera.camera_zto = argument[9];
	...
	{
		_camera.camera_xvec = argument[10];
		_camera.camera_yvec = argument[11];
		_camera.camera_zvec = argument[12];
	...
	{
		_camera.camera_velocity = argument[13];
	}
	if(argument_count >= 16)
	{
		_camera.camera_width = argument[14];
		_camera.camera_height = argument[15];
